...
/*********************************************************************************************
Who Is Active? v11.16 (2016-10-18)
		...
		BEGIN;
			...
			(
				SELECT
					...
					CONVERT(VARCHAR(8000), CONCAT('.', session_id, '.')) AS the_path
				FROM #sessions
				WHERE
					recursion = 1
				...
				SELECT
					...
					CONVERT(VARCHAR(8000), CONCAT(b.the_path, '.', s.session_id, '.')) AS the_path
				FROM blockers AS b
				JOIN #sessions AS s ON
					s.blocking_session_id = b.session_id
					AND s.recursion = 1
					AND b.the_path NOT LIKE CONCAT('%.', s.session_id, '.%') COLLATE Latin1_General_Bin2
			)
			UPDATE s
			SET

...
-- Revised 11022012 - neh - replaced @@servername with SERVERPROPERTY('ServerName')
--						- added [JobEnabled]
--						- added criteria to eliminate disabled schedules
--						- modified column aliases
--						- modified ORDER BY
--						- modified Schedule logic
-- Revised 09132016 - neh - modified JOIN syntax to scheduling tables
...
SELECT	SERVERPROPERTY ('ServerName') as [Server]
		...
		, [JobSteps] = stepcount
		, [Enabled] = CASE msdb.dbo.sysjobs.[enabled]
							WHEN 1 THEN 'x'
							ELSE ''
						END
		, [Schedule] =

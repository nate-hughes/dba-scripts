-- Retrieve the top 5 highest average logical reads statements from Query Store within the last hour
...
SELECT	TOP (@top_n) qsp.query_id
		...
		END AS sproc
		,qsqt.query_sql_text
		,AVG(qsrt.avg_logical_io_reads) AS avg_logical_io_reads
...
GROUP BY qsp.query_id,
		qsq.object_id,
        qsqt.query_sql_text
ORDER BY AVG(qsrt.avg_logical_io_reads) DESC;
